...
28. Go to `Launch Template` Menu and click on `MyZabTemplate`.
...
35. Select template named `MyZabTemplate` ---> `Actions` ---> 'Modify template (Create New Version)'.
...
40. Go to `Launch Template` Menu and click on `MyZabTemplate`.
